...
def create_test_input(batch, height, width, channels):
    ...
    if None in [batch, height, width, channels]:
        ...
        """
            np.tile returns an array by repeating input arg
            by the shape [batch, 1, 1, channels]
            -------------------
            >>> np.tile(a, 2)
            array([0, 1, 2, 0, 1, 2])
            ----------------         
...
# the class is inherited from the tf.test.TestCase class
    """
      a test function that
      compare a given test module output to an expected output given by tester
      - with respect to even size inputs
      commented by jwkang 2018 june
    """
    ...
    """
      a test function that
      compare a given test module output to an expected output given by tester
      - with respect to odd size inputs 
      commented by jwkang 2018 june
    """
    def testSeparableConv2DSameWithInputOddSize(self):
        ...
        with self.test_session() as sess:
            print ('[tf.Test] run testSeparableConv2DSameWithInputOddSize()')
            sess.run(tf.global_variables_initializer())
            self.assertAllClose(y1.eval(), y1_expected.eval())
            self.assertAllClose(y2.eval(), y2_expected.eval())
            self.assertAllClose(y3.eval(), y3_expected.eval())
...
class XceptionNetworkTest(tf.test.TestCase):
    ...
    def _xception_small(self,
        ...
        blocks = [
            ...
            block('exit_flow/block2',
                  ...
                  num_units=1,
                  stride=1),]
        return xception.xception(inputs,blocks=blocks,
    ...
    """
        ...
        the "name" and "shape" of the classification end point".
    """
    ...
    def testEndpointNames(self):
        ...
        print ('[tf.Test] run testEndpointNames()')
    ...
    def testClassificationShapes(self):
        ...
        print ('[tfTest] run testClassificationShapes()')
    ...
    def testFullyConvolutionalEndpointShapes(self):
        ...
        print ('[tfTest] run testFullyConvolutionalEndpointShapes()')
        for endpoint, shape in six.iteritems(endpoint_to_shape):
            self.assertListEqual(end_points[endpoint].get_shape().as_list(), shape)
    ...
    def testAtrousFullyConvolutionalEndpointShapes(self):
        ...
        print('[tfTest] run testAtrousFullyConvolutionalEndpointShapes()')
        for endpoint, shape in six.iteritems(endpoint_to_shape):
            self.assertListEqual(end_points[endpoint].get_shape().as_list(), shape)
    ...
    def testAtrousFullyConvolutionalValues(self):
        ...
        nominal_stride = 32
        print('[tfTest] run testAtrousFullyConvolutionalValues()')
    ...
    def testFullyConvolutionalUnknownHeightWidth(self):
        ...
        with self.test_session() as sess:
            print('[tfTest] run testFullyConvolutionalUnknownHeightWidth()')
            sess.run(tf.global_variables_initializer())
    ...
    def testAtrousFullyConvolutionalUnknownHeightWidth(self):
        ...
        with self.test_session() as sess:
            print('[tfTest] run testAtrousFullyConvolutionalUnknownHeightWidth()')
            sess.run(tf.global_variables_initializer())
            output = sess.run(output, {inputs: images.eval()})
    ...
    def testEndpointsReuse(self):
        ...
        print('[tfTest] run testEndpointsReuse()')
